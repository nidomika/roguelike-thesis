		...
		for x in range(leaf.size.x):
			for y in range(leaf.size.y):
				if not is_inside_padding(x,y, leaf, padding):
					tilemap.set_cells_terrain_connect(0, [Vector2i(x + leaf.position.x,y + leaf.position.y)], 0, 1)
		for path in paths:
			if path['left'].y == path['right'].y:
				# horizontal
				for i in range(path['right'].x - path['left'].x):
					tilemap.set_cells_terrain_connect(0, [Vector2i(path['left'].x+i,path['left'].y)], 0, 1)
			else:
				# vertical
				for i in range(path['right'].y - path['left'].y):
					tilemap.set_cells_terrain_connect(0, [Vector2i(path['left'].x,path['left'].y+i)], 0, 1)
